...
@section('content')
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0"><i class="bi bi-pencil-square me-2"></i> ویرایش دوره</h5>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form method="POST" action="{{ route('admin.courses.update', $course->id) }}" id="course-form" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                @php
                    $isFederationCourse = $course->federation_course_id ? '1' : '0';
                    $createNewTeacher = '0'; // Default to select existing
                @endphp
                ...
                <div class="mb-3">
                    ...
                    <select name="is_federation_course" id="is_federation_course" class="form-select">
                        <option value="0" {{ old('is_federation_course', $isFederationCourse) == '0' ? 'selected' : '' }}>خیر</option>
                        <option value="1" {{ old('is_federation_course', $isFederationCourse) == '1' ? 'selected' : '' }}>بله</option>
                    </select>
                </div>

                <div id="federation_section" class="row g-3 mb-4" style="display: {{ $isFederationCourse == '1' ? 'block' : 'none' }};">
                    <div class="col-md-6">
                        ...
                        <select name="federation_course_id" id="federation_course_id" class="form-select">
                            ...
                            @foreach($federationCourses as $fc)
                                <option value="{{ $fc->id }}" {{ old('federation_course_id', $course->federation_course_id) == $fc->id ? 'selected' : '' }}>
                                    {{ $fc->title }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div id="prerequisites_section" class="mb-4" style="display: {{ $isFederationCourse == '1' ? 'block' : 'none' }};">
                    <label class="form-label">پیش‌نیازها</label>
                    <select name="prerequisites[]" id="prerequisites" class="form-select" multiple>
                        @foreach($federationCourses as $fc)
                            <option value="{{ $fc->id }}" {{ in_array($fc->id, old('prerequisites', $currentPrerequisites)) ? 'selected' : '' }}>
                                {{ $fc->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="create_new_teacher" id="create_new_teacher" class="form-select">
                        <option value="0" {{ old('create_new_teacher', $createNewTeacher) == '0' ? 'selected' : '' }}>انتخاب از مدرسین موجود</option>
                        <option value="1" {{ old('create_new_teacher') == '1' ? 'selected' : '' }}>ایجاد مدرس جدید</option>
                    </select>
                </div>

                <div id="teacher_select_section" class="row g-3 mb-4" style="display: {{ old('create_new_teacher', $createNewTeacher) == '0' ? 'block' : 'none' }};">
                    <div class="col-md-12">
                ...
                <div id="teacher_create_section" class="row g-3 mb-4" style="display: {{ old('create_new_teacher') == '1' ? 'block' : 'none' }};">
                    <div class="col-md-6">
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <input type="text" name="start_date" id="start_date" class="form-control" data-jdp 
                                   value="{{ old('start_date', $course->start_date ? verta($course->start_date)->format('Y/m/d') : '') }}" required autocomplete="off">
                            <span class="input-group-text"><i class="bi bi-calendar"></i></span>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <input type="text" name="end_date" id="end_date" class="form-control" data-jdp 
                                   value="{{ old('end_date', $course->end_date ? verta($course->end_date)->format('Y/m/d') : '') }}" required autocomplete="off">
                            <span class="input-group-text"><i class="bi bi-calendar"></i></span>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="duration" id="duration" class="form-control" value="{{ old('duration', $course->duration) }}" min="0">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="time" name="start_time" class="form-control" 
                               value="{{ old('start_time', $course->start_time ? \Carbon\Carbon::parse($course->start_time)->format('H:i') : '') }}">
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">ساعت پایان</label>
                        <input type="time" name="end_time" class="form-control" 
                               value="{{ old('end_time', $course->end_time ? \Carbon\Carbon::parse($course->end_time)->format('H:i') : '') }}">
                    </div>
                </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="place" class="form-control" value="{{ old('place', $course->place) }}">
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">آدرس</label>
                        <input type="text" name="place_address" class="form-control" value="{{ old('place_address', $course->place_address) }}">
                    </div>

                    <input type="hidden" name="place_lat" id="place_lat" value="{{ old('place_lat', $course->place_lat) }}">
                    <input type="hidden" name="place_lon" id="place_lon" value="{{ old('place_lon', $course->place_lon) }}">
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="number" name="capacity" class="form-control" value="{{ old('capacity', $course->capacity) }}" min="1">
                ...
                <div class="mb-3">
                    ...
                    <select name="is_free" id="is_free" class="form-select">
                        <option value="0" {{ old('is_free', $course->is_free ? '1' : '0') == '0' ? 'selected' : '' }}>خیر</option>
                        <option value="1" {{ old('is_free', $course->is_free ? '1' : '0') == '1' ? 'selected' : '' }}>بله</option>
                    </select>
                </div>

                <div id="cost_section" class="row g-3 mb-4" style="display: {{ $course->is_free ? 'none' : 'block' }};">
                    <div class="col-md-6">
                        <label class="form-label">هزینه برای اعضا (ریال) <span class="text-danger">*</span></label>
                        <input type="text" name="member_cost" id="member_cost" class="form-control cost-input" 
                               value="{{ old('member_cost', $course->member_cost ? number_format($course->member_cost) : '') }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="guest_cost" id="guest_cost" class="form-control cost-input" 
                               value="{{ old('guest_cost', $course->guest_cost ? number_format($course->guest_cost) : '') }}" required>
                        <small class="text-muted">فقط اعداد انگلیسی مجاز است</small>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">شماره کارت <span class="text-danger">*</span></label>
                        <input type="text" name="card_number" class="form-control" value="{{ old('card_number', $course->card_number) }}" required>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">شماره شبا <span class="text-danger">*</span></label>
                        <input type="text" name="sheba_number" class="form-control" value="{{ old('sheba_number', $course->sheba_number) }}" required>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">نام دارنده حساب <span class="text-danger">*</span></label>
                        <input type="text" name="card_holder" class="form-control" value="{{ old('card_holder', $course->card_holder) }}" required>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">نام بانک <span class="text-danger">*</span></label>
                        <input type="text" name="bank_name" class="form-control" value="{{ old('bank_name', $course->bank_name) }}" required>
                    </div>
                </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select" required>
                            <option value="draft" {{ old('status', $course->status) == 'draft' ? 'selected' : '' }}>پیش‌نویس</option>
                            <option value="published" {{ old('status', $course->status) == 'published' ? 'selected' : '' }}>منتشر شده</option>
                            <option value="completed" {{ old('status', $course->status) == 'completed' ? 'selected' : '' }}>تکمیل شده</option>
                            <option value="canceled" {{ old('status', $course->status) == 'canceled' ? 'selected' : '' }}>لغو شده</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="is_registration_open" class="form-select">
                            <option value="1" {{ old('is_registration_open', $course->is_registration_open ? '1' : '0') == '1' ? 'selected' : '' }}>بله</option>
                            <option value="0" {{ old('is_registration_open', $course->is_registration_open ? '1' : '0') == '0' ? 'selected' : '' }}>خیر</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="input-group">
                            <input type="text" name="registration_deadline" id="registration_deadline" class="form-control" data-jdp data-jdp-time="true" 
                                   value="{{ old('registration_deadline', $course->registration_deadline ? verta($course->registration_deadline)->format('Y/m/d H:i') : '') }}" autocomplete="off">
                            <span class="input-group-text"><i class="bi bi-calendar"></i></span>
                        </div>
...
<!--
    ...
    <script>
        $(document).ready(function() {
            ...
            // Initialize Leaflet map for course location
            function initCourseLocationMap() {
                try {
                    const defaultLat = 35.8327; // Golshahr, Karaj
