...
<div class="container-fluid py-3">
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <div>
            ...
            <div class="text-muted small">کاربر: {{ trim(($ticket->user->profile->first_name ?? '') . ' ' . ($ticket->user->profile->last_name ?? '')) ?: $ticket->user->phone }}</div>
        ...
        <div class="card-body">
            ...
            <div class="vstack gap-3">
                @foreach($ticket->messages as $msg)
                    <div class="p-3 rounded border" style="background: {{ $msg->sender_role === 'admin' ? '#eef4ff' : '#ffffff' }};">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div class="fw-bold">
                                {{ $msg->sender_role === 'admin' ? 'ادمین' : 'کاربر' }}
                            </div>
                            ...
                            <div class="mt-2">
                                ...
                                <div class="d-flex flex-wrap gap-2">
                                    @foreach($msg->attachments as $att)
                                        <a class="btn btn-sm btn-outline-secondary" href="{{ route('admin.tickets.attachments.download', $att->id) }}">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <h6 class="mb-3">پاسخ ادمین</h6>
            <form method="POST" action="{{ route('admin.tickets.reply', $ticket->id) }}" enctype="multipart/form-data" id="admin-ticket-reply-form">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label class="form-label">پیوست‌ها (اختیاری - حداکثر ۱۰ فایل، ۵ مگابایت)</label>
                    <div class="attachment-box" id="admin-ticket-attachments">
                        <input type="file" name="attachments[]" class="d-none" id="admin-ticket-attachment-input" multiple accept="image/*,.pdf,.doc,.docx,.xlsx,.xls,.txt,.zip">
                        <div class="attachment-drop" tabindex="0" role="button" aria-label="اضافه کردن فایل" id="admin-ticket-attachment-drop">
                        ...
                        <div class="attachment-list row g-2 mt-2" id="admin-ticket-attachment-list"></div>
                        <div class="file-error"></div>
                        <small class="text-muted d-block mt-1">حداکثر ۱۰ فایل، هر کدام تا ۵ مگابایت</small>
                    </div>
                </div>
                <div class="text-end">
...
<script>
(function(){
    const container = document.getElementById('admin-ticket-attachments');
    if (!container) return;
    const input = container.querySelector('#admin-ticket-attachment-input');
    const drop = container.querySelector('#admin-ticket-attachment-drop');
    const list = container.querySelector('#admin-ticket-attachment-list');
    ...
    const addFiles = (files) => {
        ...
        if (selected.length + incoming.length > maxFiles) {
            showError('حداکثر ۱۰ فایل می‌توانید انتخاب کنید.');
            return;
        }
